...
public class LineMeshDraw2: MonoBehaviour
{
    ...
    void Start()
    {
        ...
        spherePoint = Instantiate(GameObject.CreatePrimitive(PrimitiveType.Sphere), new Vector3(0f, 0f, 0f), Quaternion.Euler(0f, 0f, 0f));
        spherePoint.transform.localScale = new Vector3(0.02f, 0.02f, 0.02f);
        spherePoint.GetComponent<MeshRenderer>().material.color = Color.green;

        spherePoint2 = Instantiate(GameObject.CreatePrimitive(PrimitiveType.Sphere), new Vector3(0f, 0f, 0f), Quaternion.Euler(0f, 0f, 0f));
        spherePoint2.transform.localScale = new Vector3(0.01f, 0.01f, 0.01f);
        spherePoint2.GetComponent<MeshRenderer>().material.color = Color.red;
    }
    ...
    void Update()
    {
        ...
        if (triggerDown && isMarker)
        {
            if (drawing)
            {
                ...
                if (Physics.Raycast(GetMarkerTip(), transform.TransformDirection(Vector3.forward), out raycastHit, 4f, layerMask) && raycastHit.collider.gameObject.tag == drawnOnObjectTag && finishedInterPoints)
                {
                    pencilTarget.SetActive(true);
                    Vector3 newPos = raycastHit.point;
                    ...
                    if ((Vector3.Distance(lastPoint, raycastHit.point) > distThreshold) && Vector3.Distance(lastPoint, raycastHit.point) > 0.05f)
                    {
                        Debug.Log("multiple points");
                        if (!gotLastRaycast)
                        {
                            StartCoroutine(AddOneMeshPoint(mesh, raycastHit));
                            gotLastRaycast = true;
                        }
                        else
                        {
                            Debug.Log("big jump!");
                            finishedInterPoints = false;
                            StartCoroutine(AddIntermediateMeshPoints(mesh, raycastHit, lastRaycast));
                        }
                    }
                    else if ((Vector3.Distance(lastPoint, raycastHit.point) > distThreshold))
                    {
                        Debug.Log("one point");
                        if (!gotLastRaycast)
                        {
                            AddNewMeshPoint(ref mesh, lastRaycast);
                            StartCoroutine(AddOneMeshPoint(mesh, raycastHit));
                        } 
                        // AddNewMeshPoint(ref mesh, raycastHit);
                        else if (gotLastRaycast && finishedInterPoints)
                        {
                            AddNewMeshPoint(ref mesh, lastRaycast);
                            // StartCoroutine(AddOneMeshPoint(mesh, lastRaycast));
                            ...
                            // if ((Vector3.Distance(lastPoint, raycastHit.point) > 1f))
                            // {
                            //     Debug.Log("big jump!");
                            //     StartCoroutine(AddIntermediateMeshPoints(mesh, lastRaycast));
                            // }
                        }
                        // Vector3 newPoint = raycastHit.point + raycastHit.normal * lineHoverAmount;
                        // secondLastPoint = lastPoint;
    ...
    IEnumerator AddOneMeshPoint(Mesh mesh, RaycastHit raycastHit)
    {
        // AddNewMeshPoint(ref mesh, lastRaycast);
        Debug.Log("adding one mesh point");
    ...
    IEnumerator AddIntermediateMeshPoints(Mesh mesh, RaycastHit currRaycastHit, RaycastHit lastRaycastHit)
    {
        float distance = Vector3.Distance(currRaycastHit.point, lastRaycastHit.point);
        // Debug.Log("distance is: " + distance);
        List<Vector3> interPoints = new List<Vector3>();
        interPoints.Add(lastRaycastHit.point);
        Vector3 lastInterPoint = lastRaycastHit.point;
        while (distance > 0.05f)
        {
            Vector3 interPoint = LerpByDistance(lastInterPoint, currRaycastHit.point, 0.1f);
            interPoints.Add(interPoint);
            lastInterPoint = interPoint;
            distance = Vector3.Distance(currRaycastHit.point, lastInterPoint);
        }
        ...
        if (interPoints.Count > 0)
        {
            ...
            {
                Vector3 interPoint = interPoints[i];
                // Debug.Log("interpoint is: " + interPoint);
                List<Vector3> vertices = new List<Vector3>(mesh.vertices);
                List<Vector3> normals = new List<Vector3>(mesh.normals);
                List<int> triangles = new List<int>(mesh.triangles);

                // get perpendicular to direction we are drawing
                Vector3 tangent = Vector3.Cross(interPoint - secondLastPoint, lastRaycastHit.normal).normalized * lineWidth * (scale / toothScaleSlider.maxValue) / 2f;
                int lastVertexIndex = mesh.vertexCount - 1;

                Vector3 leftVertexPoint = container.transform.InverseTransformPoint(interPoint + tangent + lastRaycastHit.normal * lineHoverAmount * (0.5f * scale));
                Debug.Log("left: " + leftVertexPoint);
                Vector3 rightVertexPoint = container.transform.InverseTransformPoint(interPoint - tangent + lastRaycastHit.normal * lineHoverAmount * (0.5f * scale));
                ...
                if (mesh.vertexCount == 0)
                {
                    ...
                    normals.Add(lastRaycastHit.normal);
                    normals.Add(lastRaycastHit.normal);
                    lastVertexIndex = 1;
                }
        ...
        yield return finishedInterPoints = true;
    }
    ...
    private void AddNewMeshPoint(ref Mesh mesh, RaycastHit raycastHit)
    {
        ...
        // get perpendicular to direction we are drawing
        Vector3 tangent = Vector3.Cross(raycastHit.point - secondLastPoint, raycastHit.normal).normalized * lineWidth * (scale / toothScaleSlider.maxValue) / 2f;
        int lastVertexIndex = mesh.vertexCount - 1;
    ...
    /// <summary>
    /// Function to find a Vector3 point that is x-distance away from point a, along the line a to b.
    /// </summary>
    private Vector3 LerpByDistance(Vector3 a, Vector3 b, float x)
    {
        Debug.Log("a: " + a);
        Debug.Log("b: " + b);
        Vector3 v = x * Vector3.Normalize(b - a) + a;
        Debug.Log("v: " + v);
        return v;
    }
    #endregion
